...
use tonic::{server::NamedService, transport::Server, Request, Response, Status,async_trait, Result};
...
impl  MyBitcoinService {
    pub fn new(cache: Arc<Mutex<Cache>>,receive_address_sender: Sender<ReceiveAddress>) -> MyBitcoinService{
        MyBitcoinService{
            cache,
    ...
    async fn generate(&self,request: Request<ExchangeRequest>,) ->  Result<Response<PaymentUriReply>, Status> {
        ...
        {            
            if let Result::Ok(mut cache) = cache_ref.lock() {
                ...
                let status = Status::internal("Internal server error , failed to access the cache!");
                return Err(status);
            }
        }
                
        let message = format!("Bridge {} BTC to {}",&req.source_token_amount,&req.target_chain);
        let payment_uri = MasterKey::new_bitcoin_receive_qrcode(receive_address.address.clone(), req.source_token_amount.clone().as_str(), "WHOOSH Bridge!",&message.as_str());
