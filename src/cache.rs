...
impl Cache {
    pub fn initialize(network: NetworkType, addr: String) -> anyhow::Result<Cache> {
        ...
        redis::cmd("SET")
            .arg("LAST_ACCOUNT_INDEX")
            .arg(account_index)
            .exec(&mut connection)?;
        redis::cmd("SET")
            .arg("LAST_ADDRESS_INDEX")
            .arg(address_index)
            .exec(&mut connection)?;
    ...
    pub fn generate_payment_receive(&mut self) -> anyhow::Result<ReceiveAddress> {
        let address = self
            .masterkey
            .new_bitcoin_receive_address(self.account_index, self.address_index);
        ...
        pipeline
            .cmd("SET")
            .arg("LAST_ACCOUNT_INDEX")
            .arg(self.account_index);
        pipeline
            .cmd("SET")
            .arg("LAST_ADDRESS_INDEX")
            .arg(self.address_index);
        ...
        let k = format!("RECEIVE_ADDRESS_{}", script);
